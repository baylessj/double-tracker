...
float Flanger::Process(float in) {
  lfo_val = ProcessLfo();
  del_.SetDelay(1.f + lfo_val + delay_);

  float line1 = del_.Read();
  del_.Write(in + line1 * feedback_);

  if (duplicate) {
    float line2 = -1.f * line1;
    return (in + line1 + line2) * 0.3333f;
  } else {
    return (in + line1) * .5f; // equal mix
  }
}
...
float Flanger::getLfoVal() { return lfo_val; }

void Flanger::setDuplicate(bool iduplicate) { duplicate = iduplicate; }
